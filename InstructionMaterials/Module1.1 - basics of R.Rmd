...
Matrices are another common data structure. Think of them as vectors forced into rectangular shapes. Notice that matrices can only contain one data type. Numeric, Character, Logical, etc.
```{r matrices}
# Use the matrix() fuction to create a 3x3 numeric matrix using the numbers 1 through 9.
# Search for the matrix() fuction under help to find the syntax for the command. 
...
# You can use indexing to access elements of a matrix. Use commas to indicate the dimensions.
# What are the following three lines doing? 
m[2]
m[2,]
m[,2]
...
# You can use the factor() function to assign chacater vectors as factors. You can also do this with the as.factor() function.
...
##### Exercise 7
Using the appraoch from above, create new factor with two levels "big" and "small" and calculate the maximum number of legs per group
```{r exercise7}
...
# To view the whole data frame in Rstudio, use the View() command
View(mtcars)
...
install.packages('palmerpenguins') # you must use quotations when installing packages
...
# Look at the structure of the dataframe with the str() command
str(penguins)

# Access elements in a dataframe just like a matrix
penguins[2,4]
penguins[2,]

# Get specific items from one column using it's name
penguins$sex[6:10]

```

##### Exercise 8
Using what we learned about grouping with factors, calculate the average penguin weight by island 
```{r exercise8}
...
```{r creatingdfs}
# Wse the data.frame() command to combine vectors of equal length into a data frame called animals
...
# R can handle mutidimensiontal data (more than 2D). Many packages are easier to use when 
...
#### R basics: Adding dimensions: lists
...
```{r lists}
# Create a named list with a character vetor, a numeric scalar (one numeric value), and a
...
We migth construct a loop where we don't know the number of elements in a list, or the number of elements in your list changes depending on external factors. Use the length() function to find out the number of elements in a list, vector, matrix or number of variables in a data frame.
```{r indexedloop} 
years <- 2010:2015
...
Use a double for loop to print two of these arrays - in what order does it print them? 

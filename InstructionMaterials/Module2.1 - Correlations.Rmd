...
between two variables. Pearson's coefficient corresponds to the ratio between the covariance of two variables and the product of their standard deviations. It is essentially a normalized measurement of the covariance, such that the result always has a value between −1 and 1. This measure can only reflect a **linear correlation** of variables.
...
Spearman's rank correlation is a non-parametric equivalent to Pearson's correlation. It is used to measure the strength and direction of association between two ranked variables. Spearman's correlation assesses **monotonic relationships (whether linear or not)**.

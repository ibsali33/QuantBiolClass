...
statstics. We will use techniques from previous sessions in order to simluate experiment 
...
ggplot(df.avg, aes(avg_per_replica)) + 
  ...
  labs(x = "Mean Vesicles per Replica",
       y = "Count")
...
Bootstrapping allows us to create experimental subsamples by sampling from the sample! This
...
# data set, lets create a sample of vesicles per cell that is subsampled using 
# bootstrapping with the same number of obervations and replicas that we used 
# before. Since bootstrapping is sampling done with replacement be sure to set 
# replace = TRUE
...
# calculate means for the bootstrapped subsamples

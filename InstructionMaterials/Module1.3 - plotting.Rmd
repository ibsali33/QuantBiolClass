...
## Intro to Plotting
One of the most powerful features of R is it's functionality with data visualization and plotting. 
Various developers in the R community have created robust tools for data visualization and plotting.
Some plottinglibraries are immensely extensive, allowing you to create nearly any kind of plot 
you would like and more. 

One way to plot is to use base R's plotting feature. This does not require the use of any library.
However, there are limits to base R's functionality and customizability which is why other plotting
libraries have been created. Here lets explore the basics of plotting in R. 
```{r plotting1}
# Call 50 random values collected from a random distribution into an object x and use the
# plot() function to visualize the data using a simple scatter plot
x <- rnorm(50)
plot(x)
...
You can add multiple layers to plots in R. This allows us to add lines on top of our scatter plot. 
This is useful when you want to look at multiple data characteristics simultaneously. Averages, distributions, variance and more. 
```{r plottinglines}
...
Adding more than two layers using R Base plot functions.
```{r multiplelayers}
plot(x)
lines(1:50,rep(0,50)) # a line at zero
lines(x, col = "green4", lty = "dotted") # a green line connecting the dots in chronological order
points(x, bg = "limegreen", pch = 21) # modifying the color of the dots to lime green
```
